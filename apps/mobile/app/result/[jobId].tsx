...
export default function ResultScreen() {
  ...
  // Claim title shown inside the orb
  // Get claim text for display inside the ring
  // Prefer the short title generated by the worker, fall back to raw input/claim
  const MAX_DISPLAY_CHARS = 200; // Maximum characters before truncation

  const displayClaimText = (() => {
    if (!isCompleted) return "";

    const fullText = analysis?.title || analysis?.rawInput || analysis?.claims?.[0]?.text || claimText || "No claim text detected";

    // For very long claims, truncate to prevent unreadably small text
    // Better to show truncated readable text than full tiny text
    if (fullText.length > MAX_DISPLAY_CHARS) {
      const truncated = fullText.substring(0, MAX_DISPLAY_CHARS);
      // Find last complete word before limit
      const lastSpace = truncated.lastIndexOf(' ');
      // Only truncate at word boundary if we're not losing too much
      return (lastSpace > MAX_DISPLAY_CHARS * 0.8 ? truncated.substring(0, lastSpace) : truncated) + '...';
    }
  ...
  // After scaling: 420 * 0.90 * 0.5 = 189px effective width (good fit for 210px ring)
  const calculateTitleFontSize = (text: string): number => {
    const length = text.length;
    // More granular breakpoints with better minimums for readability
    if (length < 20) return 48;      // Very short: large impact font
    if (length < 40) return 42;      // Short: medium-large font
    if (length < 60) return 36;      // Medium-short
    if (length < 80) return 32;      // Medium
    if (length < 100) return 28;     // Medium-long
    if (length < 140) return 24;     // Long
    if (length < 180) return 20;     // Very long
    return 18;                        // Extremely long - minimum readable
  };
                ...
                <Animated.View style={[
                  ...
                  <Text
                    style={{
                      ...
                      // 420 * 0.90 = 378px unscaled → 189px after 0.5 scale (fits well in 210px ring)
                      maxWidth: 420 * 0.90,
                      textShadowColor: 'rgba(0, 0, 0, 0.8)',
                      textShadowOffset: { width: 0, height: 2 },
                      textShadowRadius: 6,
                    }}
                    // Auto-scale to fit text in orb, but keep it readable
                    // minimumFontScale={0.4} means smallest font is 18 * 0.4 = 7.2px
                    // After container scale (0.5): 7.2 * 0.5 = 3.6px (still visible with shadows)
